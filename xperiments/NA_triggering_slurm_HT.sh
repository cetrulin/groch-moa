...
#SBATCH --ntasks=1
#SBATCH --array=0-11 # CPF
####SBATCH --array=0-999 # for batches 0-1
#SBATCH --array=0-399 # for batch ==2
###### SBATCH --array=0-69 
...
BATCH=2  # 0 for 0-999; 1 for 1000-1999; 2 for 2000-2999; 3 for 3000-3999; 4 for 4000=4799 (smaller array)
...
if [ $1 == "NA_1" ] || [ $1 == "NA_2" ] || [ $1 == "NA_1_2det" ] || [ $1 == "NA_2_2det" ]; then
    if [ $ALGO == "NA_1" ]  || [ $1 == "NA_1_2det" ]; then
        ARGS1=(1 1.25 1.5 1.75 2) # (0.5 0.75 1 1.25 1.5 1.75 2)
    fi
    if [ $ALGO == "NA_2" ] || [ $1 == "NA_2_2det" ]; then
        ARGS1=(1.5 1.75 2 2.25 2.5) # (0.5 0.75 1 1.25 1.5 1.75 2)
    fi
    ARGS2=(200 300) # (100 200 300 400 500)       
    ...
    echo "./run_parameter_optimization_NA.sh     srun NA_loops_opt_HT.sh ${ALGO} ${my_array1[$(($THREAD))]}  ${PROBLEMS} ${DATASET} "${MAHABSET}" ${my_array2[$(($THREAD))]}  ${my_array3[$(($THREAD))]} ${my_array4[$(($THREAD))]} ${my_array5[$(($THREAD))]} ${my_array6[$(($THREAD))]} ${my_array7[$(($THREAD))]} ${my_array8[$(($THREAD))]}"
    ...
    srun NA_loops_opt_HT.sh ${ALGO} ${my_array1[$(($THREAD))]}  ${PROBLEMS} ${DATASET} "${MAHABSET}" ${my_array2[$(($THREAD))]}  ${my_array3[$(($THREAD))]} "${my_array4[$(($THREAD))]}" "${my_array5[$(($THREAD))]}" "${my_array6[$(($THREAD))]}" ${my_array7[$(($THREAD))]} ${my_array8[$(($THREAD))]}
else
    # Create the job
    ...
    echo "========= TASK ID: ${SLURM_ARRAY_TASK_ID} ========="
    echo "./run_parameter_optimization_COMPETITORS.sh ${ALGO} ${CPF_ms_array1[$(($SLURM_ARRAY_TASK_ID))]} ${PROBLEMS} ${DATASET} ${MAHABSET} ${CPF_min_buffer_sizes_array2[$(($SLURM_ARRAY_TASK_ID))]}"
    echo "--------------------------"
    ...
    srun run_parameter_optimization_COMPETITORS_HT.sh ${ALGO} ${CPF_ms_array1[$(($SLURM_ARRAY_TASK_ID))]} ${PROBLEMS} ${DATASET} ${MAHABSET} ${CPF_min_buffer_sizes_array2[$(($SLURM_ARRAY_TASK_ID))]}
