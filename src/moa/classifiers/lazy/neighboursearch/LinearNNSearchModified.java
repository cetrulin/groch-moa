 ...
 */
public class LinearNNSearchModified
  ...
  protected DistanceFunction m_DistanceFunction = new EuclideanDistanceModified();
   ...
   */
  public LinearNNSearchModified(Instances insts) {
    super(insts);
   ...
   * @throws Exception  if the neighbours could not be found.
   */
  public Instances kNearestNeighboursUsingClass(Instance target, int kNN) throws Exception {
    ...
    for(int i=0; i<m_Instances.numInstances(); i++) {
      ...
      if(firstkNN<kNN) {
        if(print)
          System.out.println("K(a): "+(heap.size()+heap.noOfKthNearest()));
        distance = ((EuclideanDistanceModified) m_DistanceFunction).distanceWithClass(target, m_Instances.instance(i), Double.POSITIVE_INFINITY, true);
        if(distance == 0.0 && m_SkipIdentical)
          if(i<m_Instances.numInstances()-1)
            continue;
          else
        ...
        if(print)
          System.out.println("K(b): "+(heap.size()+heap.noOfKthNearest()));
        distance = ((EuclideanDistanceModified) m_DistanceFunction).distanceWithClass(target, m_Instances.instance(i), temp.distance, true);
        if(distance == 0.0 && m_SkipIdentical)
          continue;
        if(distance < temp.distance) {
          heap.putBySubstitute(i, distance);
